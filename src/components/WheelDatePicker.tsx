...
interface WheelDatePickerProps {
  ...
  initialIsLeap?: boolean;
  onConfirm: (year: number, month: number, day: number, isLunar: boolean, isLeap: boolean) => void;
}
        ...
        <>
          ...
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#FAF9F7", borderTopLeftRadius: "24px", borderTopRightRadius: "24px", padding: "20px 20px 40px 20px", zIndex: 1001, boxShadow: "0 -4px 20px rgba(0,0,0,0.1)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              ...
              <div style={{ display: "flex", background: "rgba(0,0,0,0.05)", borderRadius: "10px", padding: "4px", gap: "4px" }}>
                ...
                {isLunar && (
                  <button onClick={() => setIsLeap(!isLeap)} style={{ border: "none", background: isLeap ? "var(--accent-indigo)" : "rgba(0,0,0,0.05)", color: isLeap ? "white" : "#666", padding: "6px 12px", borderRadius: "8px", fontSize: "0.85rem", fontWeight: "700", transition: "all 0.2s" }}>윤달</button>
                )}
              </div>
              <button onClick={() => { onConfirm(year, month, day, isLunar, isLeap); onClose(); }} style={{ border: "none", background: "transparent", color: "#C9A050", fontWeight: "700", fontSize: "1.05rem", cursor: "pointer" }}>확인</button>
            </div>
