...
- 반드시 공백 포함 **1,800자 ~ 2,500자** 범위로 작성하십시오.
- ${strategyContext}
- **극강의 구체성 (필수)**: "상반기", "하반기" 같은 느슨한 타임라인을 전면 금지합니다. 반드시 "2026년 5월 11일~6월 20일 사이"와 같이 구체적인 월/일 단위의 타임라인을 제시하십시오.
- **실무적/현실적 수치 제공 (필수)**: "안정화에 집중하라" 같은 뻔한 일반론 대신, 내담자의 고민 영역(사업, 직장 등)에 맞춘 실무적 지표를 제공하십시오. (예: "계약 체결 시 80% 이상 성공 확률", "부동산 투자 시 리스크 30% 증가", "매출 목표 120% 달성 가능")
- **반복 абсолю트 제로**: 앞선 섹션의 분석을 단 한 줄도 반복(요약)하지 마십시오. 오직 실전 타임라인과 구체적 이벤트 예측에만 집중하십시오.
- **연도별 구조화**: 2026년을 60% 이상(월/일별 승부처 명시), 2027-2028년을 40%로 배정하여 표나 불릿 포인트로 깔끔하게 구조화하십시오.
- **프리미엄 골드 강조 (필수)**: 가장 확률이 높은 승부처 시점과 구체적 지표 예측을 반드시 **[major]내용[/major]** 태그로 감싸십시오.`,
...
- 반드시 공백 포함 **1,800자 ~ 2,500자** 범위로 작성하십시오.
- ${strategyContext}
- **추상적 일반론 전면 금지**: "명상하십시오", "남서쪽 방향이 좋습니다" 같은 뻔한 조언을 배제하고, 내담자의 사업/실무/일상 현장에 즉시 적용 가능한 초구체적 행동 지침을 제공하십시오.
- **행동 체크리스트 예시**: "중요 계약서 작성 시 남서쪽 창가를 등지고 황토색 펜을 사용하면 토(土) 기운이 30% 증폭되어 계약 성공률 상승", "핵심 파트너 미팅은 매일 아침 7시~7시 15분 사이에 세팅할 것" 등 측정/실행 가능한 형태로 변환하십시오.
- **강약 조절 (수치화된 솔루션 필수)**: 위험 요소를 지적할 때 "주의하세요"로 끝내지 마십시오. "이 방식을 적용하면 리스크를 70% 감소시킬 수 있습니다"와 같이 구체적인 방어율 수치와 실전 대처법을 명시하십시오.
- **반복 절대 금지**: 기존 맥락을 재설명(요약)하지 말고 곧바로 1, 2, 3번 체크리스트나 불릿 포인트로 비책을 나열하십시오.
- **프리미엄 골드 강조 (필수)**: 가장 치명적인 약점을 보완할 수치화된 구체적 행동 실천법을 반드시 **[major]내용[/major]** 태그로 감싸십시오.`,
  };
