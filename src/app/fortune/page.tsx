...
function FortuneContent() {
  ...
  const typeConfig: Record<string, any> = {
    ...
    yearly: {
      ...
      tabs: ["종합", "재물", "애정", "건강", "사업"],
      keys: ["this_year", "wealth", "love", "health", "career"]
    },
    wealth: {
      title: "재물운",
  ...
  const getCacheKey = () => {
    ...
    return `fortune_v34_${date}_${time}_${isLunar}_${isLeap}_${gender}_${typeParam}_${timeModifier}`;
  };
...
`;
      ...
      if (typeParam === "wealth") {
        ...
        timeContext = `오늘은 ${todayEnhancedStr}입니다. 현재는 ${tm}월이며, 다음 달 기운은 ${nmStr}입니다. 이번 달과 다음 달의 흐름을 아래 JSON 구조로 응답하세요.

반드시 아래 JSON 객체로 응답하세요:
{
  "this_month": {
    "content": "이번 달(${tm}월) 전체 종합 운세 (핵심만 추려 공백 포함 200~300자 내외로 간결하게 서술. 가독성을 위해 **5~8문장마다** 반드시 줄바꿈(\\n\\n)을 2번 사용할 것. 반드시 포함: 핵심 키워드 3개, 전체 기운 흐름, 좋은 점, 주의할 점)",
    "score": (0-100 사이 정수),
    "last_month_score": (0-100 사이 정수),
    "next_month_score": (0-100 사이 정수),
    "scores": {"wealth":(0-100 사이 정수),"love":(0-100 사이 정수),"career":(0-100 사이 정수),"health":(0-100 사이 정수)}
  },
  "week1": { "content": "1주차 흐름 (명리적 근거를 포함하여 공백 포함 200자 이상 상세히 서술)" },
  "week2": { "content": "2주차 흐름 (명리적 근거를 포함하여 공백 포함 200자 이상 상세히 서술)" },
  "week3": { "content": "3주차 흐름 (명리적 근거를 포함하여 공백 포함 200자 이상 상세히 서술)" },
  "week4": { "content": "4주차 흐름 (명리적 근거를 포함하여 공백 포함 200자 이상 상세히 서술)" },
  "week5": { "content": "5주차 흐름 (명리적 근거를 포함하여 공백 포함 200자 이상 상세히 서술)" },
  "wealth": { "content": "이번 달 재물운 (공백 포함 200자 이상 상세히 서술)", "score": (0-100 사이 정수) },
  "love": { "content": "이번 달 애정운 (공백 포함 200자 이상 상세히 서술)", "score": (0-100 사이 정수) },
  "health": { "content": "이번 달 건강운 (공백 포함 200자 이상 상세히 서술)", "score": (0-100 사이 정수) },
  "career": { "content": "이번 달 사업운 (공백 포함 200자 이상 상세히 서술)", "score": (0-100 사이 정수) },
  "lucky_days": "길일 목록",
  "unlucky_days": "흉일 목록",
...
[매우 중요: 점수 산출 가이드]
- 모든 점수는 예시(80, 85 등)를 그대로 사용하지 말고, 내담자의 일간(Day Master)과 해당 월(${tm})의 기운(천간/지지)이 만났을 때 발생하는 생극제화(生剋制化)와 합충(合沖) 관계를 면밀히 분석하여 **반드시 실시간으로 계산**하십시오.
- 지난달, 이번 달, 다음 달의 점수가 항상 똑같이 나오지 않도록 기운의 성쇠(盛衰)를 엄격히 구분하여 반영하십시오.

[필수 가이드라인]
- **부드러운 구어체 어미**: "~네요, ~지요, ~일 거예요, ~하시면 좋겠어요" 등 대화하는 듯한 친근한 종결 어미를 적극적으로 섞어서 사용하세요. (주의: "~입니다, ~음을 암시합니다" 등 건조한 표현 지양)
- **용어 통일**: '신묘달' 등 '달'이라는 표현 대신, 반드시 **'신묘월'**과 같이 **'~월'**이라는 표현을 사용하십시오.
...
- **숫자 점수 언급 금지**: 모든 텍스트 분석 내용(content) 중에는 "80점입니다", "점수가 높네요" 등 구체적인 **숫자 점수를 절대 언급하지 마십시오.** 점수는 오직 JSON의 전용 숫자 필드에만 담으세요.
...
{
  "this_year": {
    "content": "올해 총 운세 (명리적 팩트폭격 위주, 긍정 5 부정 5 통계적 비율, 개운법 포함하여 공백 포함 1000자 이상 매우 상세히 서술)",
    "score": (0-100 사이 정수),
    "last_year_score": (0-100 사이 정수),
    "next_year_score": (0-100 사이 정수),
    "monthlyEnergies": [12개월 각각의 점수 0-100 사이 12개 나열],
    "scores": {"wealth":(0-100 사이 정수),"love":(0-100 사이 정수),"career":(0-100 사이 정수),"health":(0-100 사이 정수)}
  },
  "wealth": { "content": "올해 재물운 (명리적 팩트폭격 위주, 긍정 5 부정 5 비율, 개운법 포함하여 공백 포함 500자 이상 파격적 수준으로 서술)", "score": (0-100 사이 정수) },
  "love": { "content": "올해 애정운 (명리적 팩트폭격 위주, 긍정 5 부정 5 비율, 개운법 포함하여 공백 포함 500자 이상 파격적 수준으로 서술)", "score": (0-100 사이 정수) },
  "health": { "content": "올해 건강운 (명리적 팩트폭격 위주, 긍정 5 부정 5 비율, 개운법 포함하여 공백 포함 500자 이상 파격적 수준으로 서술)", "score": (0-100 사이 정수) },
  "career": { "content": "올해 사업운 (명리적 팩트폭격 위주, 긍정 5 부정 5 비율, 개운법 포함하여 공백 포함 500자 이상 파격적 수준으로 서술)", "score": (0-100 사이 정수) },
  "lucky_months": "길월 목록",
  "unlucky_months": "흉월 목록",
  "gaewun": {"color":"색상","item":"물건","numbers":"숫자5개","direction":"방향"}
}

[매우 중요: 점수 산출 가이드]
- 모든 점수는 정형화된 숫자(80, 85 등)를 그대로 쓰지 말고, 내담자의 일간과 올해(2026년 병오년)의 명리적 상호작용을 분석하여 **매번 다르게 산출**하십시오.
- 작년, 올해, 내년의 점수가 기운의 변화에 따라 반드시 유의미하게 차이나도록 하십시오.
- monthlyEnergies 배열은 1월부터 12월까지의 기운 세기를 각각 독립적으로 분석하여 12개의 숫자로 채우십시오.
...
- **가독성(문단 나누기)**: 빽뻑한 느낌이 들지 않게 **5~8문장마다** 반드시 줄바꿈(\\n\\n)을 2번 사용하여 문단을 시원하게 나누어 주세요.
- **숫자 점수 언급 금지**: 모든 텍스트 분석 내용(content) 중에는 "85점입니다"와 같은 구체적인 **숫자 점수를 절대 언급하지 마십시오.** 점수는 JSON의 숫자 필드에만 기록하세요.
...
반드시 아래 JSON 객체로 응답하세요:
{
  "yesterday": { "score": (0-100 사이 정수) },
  "today": { 
    "content": "오늘 종합운 핵심 풀이 (명리적 근거를 바탕으로 공백 포함 300~400자 이상 상세히 서술)", 
    "score": (0-100 사이 정수),
    "scores": {"wealth":(0-100 사이 정수),"love":(0-100 사이 정수),"career":(0-100 사이 정수),"health":(0-100 사이 정수)},
    "gaewun": {"color":"색상","item":"행운의 물건","numbers":"1~99숫자5개","direction":"방향"}
  },
  "tomorrow": { "score": (0-100 사이 정수) },
  "wealth": { "content": "오늘 재물운 (공백 포함 150자 이상 상세히 서술)", "score": (0-100 사이 정수) },
  "love": { "content": "오늘 애정운 (공백 포함 150자 이상 상세히 서술)", "score": (0-100 사이 정수) },
  "health": { "content": "오늘 건강운 (공백 포함 150자 이상 상세히 서술)", "score": (0-100 사이 정수) },
  "career": { "content": "오늘 사업운 (공백 포함 150자 이상 상세히 서술)", "score": (0-100 사이 정수) }
}

[매우 중요: 점수 산출 가이드]
- 예시 숫자를 복제하지 말고, 오늘/어제/내일의 일진(Hae-un)과 내담자의 만세력이 충돌하거나 상생하는 정도를 점수화하십시오.
- 각 영역별 점수(scores)와 개별 키(wealth, love 등)에 포함된 score는 서로 연동되면서도 동시에 해당 영역에 특화된 수치로 산출되어야 합니다.
- 어제, 오늘, 내일의 점수가 항상 동일하게 나오지 않도록 날짜별 기운의 차이를 확실히 반영하십시오.
- **숫자 점수 언급 금지**: 모든 텍스트 분석 내용(content) 중에는 "75점입니다"와 같은 구체적인 **숫자 점수를 절대 언급하지 마십시오.** 점수는 오직 JSON의 전용 숫자 필드에만 담으세요.
- 마크다운/HTML/영어 금지. 인사말 없이 바로 깊이 있는 풀이 시작.`;
      } else {
...
[가장 중요한 작성 원칙: 밀도와 깊이]
1. "좋을 수도 있고 나쁠 수도 있습니다", "항상 긍정적으로 생각하세요" 같은 뻔한 잡설(Filler)을 단 한 줄도 쓰지 마세요.
2. 짧은 분량이라도 오직 명리학적 근거(십신, 신살, 오행의 생극제화 등)에 기반하여 '왜(Why)' 그런 운의 흐름이 나오는지 팩트 위주로 밀도 있게 서술하세요.
3. 흉운이나 약점은 절대 숨기거나 미화하지 말고 냉철하게 분석하세요.
4. **6:4 길흉 밸런스**: 무조건 좋은 말만 늘어놓지 마십시오. 모든 세션에 걸쳐 긍정적 희망 60%와 현실적 주의(흉운) 40%의 비율을 완벽히 지키십시오.
5. **세션 간 단어 중복 100% 차단**: 일간/월간/연간 및 각 테마(재물, 건강 등)마다 완벽히 다른 고품격 어휘와 비유를 사용하십시오. 똑같은 단어, 똑같은 문장 구조가 반복되면 최악의 오류로 간주합니다.
7. **가독성 최적화(문단 나누기)**: 모든 서술형 텍스트는 **5~8문장마다** 반드시 줄바꿈(\\n\\n)을 2번 사용하여 문단을 시원하게 나누어 주십시오. 텍스트가 덩어리진 느낌이 들지 않게 하십시오.
반드시 JSON 형식으로 응답하며,
...
- 동네 철학관이 아닌, 군더더기 없이 핵심만 찌르는 진정한 대가(大家)의 어조로 작성하세요. 말을 늘이지 마십시오.
- 2026년 병오년을 기준으로 내담자가 처한 명리적 환경(세운과 원국의 상호작용)을 낭비되는 단어 없이 정밀하게 분석하십시오.
...
{
  "summary": "(해당 시기의 재물운 핵심 총평 요약 및 명리적 이유 포함, 공백 포함 3500~4000자 이상 압도적이고 방대한 분량으로 상세 서술)",
  "cards": {
    "focus": { "title": "핵심 재물원 등 한 줄 요약", "content": "어디서 돈이 들어오는지 구체적 지침 (공백 포함 1600자 이상 상세히)" },
    "expense": { "title": "지출 방어 등 한 줄 요약", "content": "돈이 새어나가는 곳과 대비책 (공백 포함 1600자 이상 상세히)" },
    "partners": { "title": "재물 귀인 등 한 줄 요약", "content": "재물운을 돕는 인연이나 환경 (공백 포함 1600자 이상 상세히)" },
    "risk": { "title": "투자 리스크 등 한 줄 요약", "content": "투자와 관련된 주의사항 (공백 포함 1600자 이상 상세히)" }
  },
  "action_tips": {
...
# Quality: Use natural, professional, high-quality Korean prose. Avoid weird, literal, or overly technical phrasings. Ensure the consultation feels premium and insightful.
{
  "summary": "(해당 시기의 건강운 핵심 총평 요약 및 명리적 근거 포함, 공백 포함 3500~4000자 이상 압도적이고 방대한 분량으로 상세 서술)",
  "cards": {
    "physical": { "title": "핵심 신체 기운 등 한 줄 요약", "content": "주의해야 할 신체 부위나 강화해야 할 체력 요소 (공백 포함 1600자 이상 상세히)" },
    "mental": { "title": "정신 건강 지수 등 한 줄 요약", "content": "스트레스 관리나 마음의 여유를 위한 조언 (공백 포함 1600자 이상 상세히)" },
    "lifestyle": { "title": "생활 습관 제안 등 한 줄 요약", "content": "이 시기 꼭 지켜야 할 식습관이나 활동 지침 (공백 포함 1600자 이상 상세히)" },
    "habits": { "title": "운동 및 휴식 등 한 줄 요약", "content": "권장하는 운동 강도나 휴식의 방식 (공백 포함 1600자 이상 상세히)" }
  },
  "action_tips": {
...
{
  "${currentType.keys[0]}": {
    "summary": "(총론: 현재 사업운의 핵심 총평과 방향성 및 명리적 이유 요약, 공백 포함 4000~5000자 이상 압도적이고 방대한 분량으로 상세 서술)",
    "energy": [
    ...
    "cards": {
      "timing": { "title": "가을 (8~10월) 등 한 줄 요약", "content": "시기별 구체적 행동 지침 (공백 포함 2000자 이상 상세히)" },
      "partners": { "title": "핵심 파트너 등 한 줄 요약", "content": "협력관계를 맺어야 할 상대나 주의해야 할 상대 (공백 포함 2000자 이상 상세히)" },
      "capital": { "title": "보수적 운용 등 한 줄 요약", "content": "사업적 자금 순환 및 지출 관리 전략 (공백 포함 2000자 이상 상세히)" },
      "risk": { "title": "계약 검토 등 한 줄 요약", "content": "발생 가능한 리스크와 대비책 (공백 포함 2000자 이상 상세히)" }
    },
    "action_tips": {
      "do": "가장 중요한 실천 지침 1가지",
...
# Quality: Use natural, professional, high-quality Korean prose. Avoid weird, literal, or overly technical phrasings. Ensure the consultation feels premium and insightful.
{
  "summary": "(해당 시기의 애정운 핵심 총평 요약 및 명리적 이유 포함, 공백 포함 3500~4000자 이상 압도적이고 방대한 분량으로 상세 서술)",
  "cards": {
    "romance": { "title": "연애 에너지 성향 등 한 줄 요약", "content": "이 시기의 전반적인 연애 기운과 태도 (공백 포함 1600자 이상 상세히)" },
    "partners": { "title": "이상적인 인연 등 한 줄 요약", "content": "어떤 사람과 합이 좋은지, 인연이 닿는 시기 (공백 포함 1600자 이상 상세히)" },
    "caution": { "title": "관계의 주의점 등 한 줄 요약", "content": "갈등 요소나 경계해야 할 태도 (공백 포함 1600자 이상 상세히)" },
    "timing": { "title": "인연의 타이밍 등 한 줄 요약", "content": "결혼이나 깊은 관계로 발전하기 좋은 시기 (공백 포함 1600자 이상 상세히)" }
  },
  "action_tips": {
                  ...
                  {typeParam === "daily" ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
                      ...
                      {typeParam === "monthly" && (
                        <>
                          <div>
                            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px" }}><div style={{ width: "4px", height: "18px", background: "#81b29a", borderRadius: "2px" }} /><h3 style={{ fontSize: "1.15rem", fontWeight: "700" }}>📆 주차별 운세</h3></div>
                            {["week1","week2","week3","week4","week5"].map((key, idx) => {
                              const now = new Date();
                              const curDate = now.getDate();
                              const curWeekIdx = Math.min(4, Math.floor((curDate - 1) / 7));
                              const isCurWeek = idx === curWeekIdx;
