...
function extractDataRegex(raw: string): any {
    ...
    const result: any = {
        ...
        subheadline: "귀하의 운명을 관통하는 통찰입니다.",
        yongsin: "분석중",
...
{
  "headline": "귀하의 질문을 명리학적으로 풀어낸 답변입니다.",
  "subheadline": "귀하의 현재 상황을 관통하는 통찰 (한 문장)",
...
당신은 명리 대가로서 본격적인 서술에 앞서, 귀하의 고민([Answers])과 사주 데이터([Data])를 바탕으로 흔들리지 않는 **'최종 분석 전략'**을 수립해야 합니다.
...
3. **핵심 조언**: 귀하가 가장 주의해야 할 점이나 개운의 핵심 방침을 정하십시오.
...
- **배타적 영역**: 오직 귀하의 타고난 원국의 기운, 성정, 인생의 전체적인 형상을 비유적으로 묘사하는 데에만 집중하십시오. (미래 시기 예측 금지)
- **프리미엄 골드 강조**: 핵심 문장을 반드시 **[major]내용[/major]** 태그로 감싸십시오.
- 출력 포맷: JSON {"life_shape": "내용"}`,

      3: isTotalFortune ? `### [섹션 2: 성격 및 사회적 성향]
${volumeRule}
- **데이터 분석 필수**: 귀하의 사주에 나타난 사회적 재능(십성)과 활동성(12운성)을 근거로 입체적인 분석을 제공하십시오.
- **배타적 영역**: 십성과 오행의 분포를 기반으로 귀하의 본모습과 사회적 재능을 입체적으로 분석하는 데 집중하십시오.
...
- **질문 해결 중심의 7:3 황금 비율 (핵심)**: 분석의 70%는 귀하의 고민("${userAnswers[0]}")에 대한 **직설적인 해답과 실천적 솔루션**으로 채우십시오. 나머지 30%만 명리학적 근거(십성, 운의 흐름 등)를 설명하는 데 사용하십시오.
- **실전 액션 플랜**: 추상적 풀이가 아니라, 질문과 관련된 **당장 실행 가능한 구체적 행동(예: 투자 보류, 이직 시기 조정 등)**을 명확히 지시하십시오.
- **배타적 영역**: 귀하의 구체적인 고민에 대해 직접적이고 [major]명쾌한 해답[/major]을 제시하는 데 완전히 집중하십시오. 다른 섹션의 내용을 반복하지 마십시오.
- **도입부**: 첫 문장은 질문을 직접 언급하며 공감을 표하며 시작하십시오.
- 출력 포맷: JSON {"solution": "내용"}`,

      4: isTotalFortune ? `### [섹션 3: 분야별 상세 인생 운세]
${volumeRule}
- **데이터 분석 필수**: 분야별로 관련된 십성과 길성/흉살의 작용을 평생의 흐름 속에서 정밀하게 분석하십시오.
- **배타적 영역**: 특정 날짜나 연도를 언급하지 말고, 귀하의 평생 운세를 각 분야별로 깊이 있게 분석하는 데만 집중하십시오.
...
- **배타적 영역**: 귀하가 선택한 메뉴("${category || '선택 영역'}")를 중심으로 3년간의 운의 흐름과 타이밍 분석에만 집중하십시오.
...
- 출력 포맷: JSON {"timing": "내용", "luck_advice": "내용"}`,
    ...
    promptText = `
### [전문가 정체성: 정통 명리학의 거장 - 냉혹한 현실론자]
당신은 수십 년간 수만 명의 운명을 감명해 온 **정통 명리학의 거장**입니다. 당신은 귀하에게 입에 발린 위로나 헛된 희망을 절대 주지 않는 **'냉혹하고 뼈 때리는 스승'**의 관점에서 서술하십시오. 안 좋을 때는 안 좋다고 가차 없이 팩트 폭격을 날리는 것이 귀하를 진심으로 돕는 길임을 명심하십시오.
...
8. **가차 없는 팩트 폭격 (필수)**: 무조건 좋은 말만 하지 마십시오. 긍정적 내용 40%와 함께, 반드시 뼈 아픈 현실 직시와 주의해야 할 흉살, 재물 파산, 고립 등의 **냉혹한 경고 60%**를 조합하여 신뢰감을 주십시오. (길흉 밸런스를 40:60으로 조정하여 더 정밀하게 압박하십시오)
9. **부정 제약 조건**: 부정적인 운세 풀이 시 "다행히도", "하지만", "노력하면" 등의 근거 없는 낙관론이나 완곡한 표현 사용을 **절대 금지**합니다. 흉운은 그 자체로 매섭게 경고하십시오.
10. **섹션 간 설정(날짜) 통일 및 배타적 영역 준수**: 섹션이 달라도 길일/흉일의 날짜가 엇갈려서는 안 됩니다. 하나의 일관된 '마스터 타임라인'을 유지하십시오. 각 섹션 고유의 주제(예: 섹션1=성격, 섹션3=시기)를 엄격히 한정하고 다른 섹션 내용을 침범하거나 중복 서술하지 마십시오.
11. **핵심 명령**: 지정된 글자 수 범위(2,000~2,500자)를 철저히 엄수하고 마크다운 기호를 전면 금지하십시오. 기계적인 서론 없이 본론으로 직행하십시오.
`;
  }
