...
- ${strategyText}
- 타고난 원국의 기운, 성정을 직설적이고 통찰력 있게 분석하십시오. (시적 비유 엄금)
- 날짜나 시기는 언급하지 마십시오.
...
- 반드시 공백 포함 **1,800자 ~ 2,500자** 범위로 작성하십시오.
- ${strategyText}
- 오직 내담자의 타고난 원국 기운과 성정을 직설적이고 구조화된 방식으로 분석하는 데에만 집중하십시오. 비유적 표현은 철폐하십시오.
...
- 출력 포맷: JSON {"detailed_fortune": "내용", "turning_points": "내용"}` // 분야별 상세와 전환점
      : `### [상세 운세(timing) 및 실전 개운 비책(luck_advice)]
- 각 항목당 반드시 공백 포함 **1,800자 ~ 2,500자** 범위로 작성하십시오.
- ${strategyText}
- **[timing (3개년 상세 운세) 지침]**
  - **극강의 구체성**: "상/하반기" 같은 느슨한 표현 금지. "2026년 5월 11일~6월 20일"처럼 월/일 단위의 정확한 타임라인 제시.
  - **실무적 수치 제공**: 내담자의 고민 영역("${category || '선택 영역'}")에 맞춰 "계약 성공률 80%", "리스크 30% 증가" 등 현실적 지표 제공.
  - **연도별 구조화**: 2026년 60%, 2027-2028년 40%로 배분(불릿 포인트 적극 활용). 분석 내용 반복 절대 금지.
- **[luck_advice (실전 개운 비책) 지침]**
  - **추상적 일반론 전면 금지**: "명상하라", "남쪽이 좋다" 식의 뻔한 조언 배제.
  - **행동 체크리스트 제공**: "중요 계약서 작성 시 남서쪽 창가를 등지고 황토색 펜 사용 시 토(土) 기운 30% 증폭시켜 계약 체결률 상승" 등 즉시 실행 가능한 초구체적 지침으로 작성.
  - **강약 조절**: 위험 지적에 그치지 말고 "이 방식을 적용하면 리스크 70% 감소" 등 수치화된 방어 척도 명시. 기존 분석의 반복(요약) 없이 곧바로 체크리스트로 진입할 것.
- **프리미엄 골드 강조**: 확률 높은 시점, 구체적 지표, 치명적 약점을 보완할 실천법을 **[major]내용[/major]** 태그로 감싸십시오.
...
5. **균형 잡힌 현실적 해석**: 무조건 긍정적인 내용만 나열하지 마십시오. 위험 시기, 약점, 한계를 명확히 짚고, 모호한 "가능성" 단어 대신 "강한 흐름", "주의 필요" 등 강단을 보여주십시오. 실용적 대안("이렇게 활용하면 성과를 기대할 수 있습니다")을 제시하십시오.
6. **구체성 극대화**: "하반기에..." 같이 얼버무리지 말고 연월일(예: 2026년 7월~9월) 단위로 정확한 시기를 명시하십시오. 명식 대조 데이터를 신뢰하십시오.
7. **간결화 및 구조화**: 한 문장은 절대 2줄을 넘지 않게 간결히 끊으십시오. 단락을 쪼개고, **불릿 포인트(-), 번호 매김(1. 2.), 표 등을 적극 활용**하여 구조화된 분석을 제공하십시오. 동일한 내용 반복은 절대 불가하며 인사말은 생략하십시오.
8. **단일 호칭 통일**: 내담자를 지칭할 때는 오직 **"귀하"**라는 단어 하나로 통일하십시오.
9. **프리미엄 골드 강조 (필수)**: 핵심 시기, 구체적 장점, 주의해야 할 단점, 결정적 솔루션 등을 포함한 핵심 통찰 문장 전체를 반드시 **[major]내용[/major]** 태그로 감싸십시오. (주의: 태그 스펠링을 절대 틀리지 마십시오)
10. **핵심 명령**: 지정된 글자 수 범위를 **철저히 엄수**하십시오. 텍스트를 강조할 때 별표(**)는 오류를 유발하므로 철저히 배제하고 오직 [major]태그만 사용하십시오.
`;
  }
